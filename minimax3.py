...
def maximize(board: board_class.Board, depth: int, max_depth: int, player_color: str, computer_color: str,
    ...
    for child in children:
        ...
        if move.evaluate_2(computer_color) >= max_value.evaluate_2(computer_color):
            max_value: board_class.Board = child

        # Implementing a-b pruning
        if max_value.evaluate_2(computer_color) >= beta:
            return max_value

        if max_value.evaluate_2(computer_color) > alpha:
            alpha: int = max_value.evaluate_2(computer_color)
    return max_value
...
def minimize(board: board_class.Board, depth: int, max_depth: int, player_color: str, computer_color: str,
    ...
    for child in children:
        ...
        if move.evaluate_2(computer_color) <= min_value.evaluate_2(computer_color):
            min_value: board_class.Board = child

        # Implementing a-b pruning
        if min_value.evaluate_2(computer_color) <= alpha:
            return min_value

        if min_value.evaluate_2(computer_color) < beta:
            beta: int = min_value.evaluate_2(computer_color)
    return min_value
